...
all_param_df.to_csv(MODEL_DIR / f'benth_oos_param_df_{tdy}.csv', index=False)
summary_df = pd.DataFrame(summary_rows)
summary_df.to_csv(MODEL_DIR / 'benth_model_summary.csv', index=False)
...
if len(diag_store) > 0:
    ...
    fig.tight_layout()
    fig.savefig(MODEL_DIR / 'benth_model_diagnostics.png', bbox_inches='tight', dpi=150)
    plt.close(fig)
