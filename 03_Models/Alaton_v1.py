...
- while exporting one PKL per region per calibration year:
    01_PKL Files/02_Alaton/alaton_model_{region}_calib_{year}.pkl
...
summary_df.to_csv(MODEL_DIR / 'alaton_model_summary.csv', index=False)
...
if len(diag_store) > 0:
    ...
    fig.tight_layout()
    fig.savefig(MODEL_DIR / 'alaton_model_diagnostics.png', bbox_inches='tight', dpi=150)
    plt.close(fig)
