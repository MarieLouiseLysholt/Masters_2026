...
BK_FORECASTS_CSV = SCRIPT_DIR / "bk_forecasts.csv"
BK_OBS_METADATA_CSV = SCRIPT_DIR / "bk_obs_metadata.csv"
...
def _recursive_forecast(predict_fn, working_df, test_dates, trend_int, trend_slope, fourier_coefs,
    ...
    for i in range(n_steps):
        ...
        if clip_bound is not None:
            dts_pred = float(np.clip(dts_pred, -clip_bound, clip_bound))
        try:
...
for lbl, rd in [("CAT", rd_cat), ("HDD", rd_hdd), ("CDD", rd_cdd)]:
    ...
    print(f"\n# RANK COUNT — {lbl} APE\n{rc.to_string()}")
    print(f"\n# AVG RANK — {lbl} APE\n{ar.to_string()}")
    rc.to_csv(output_path(f"bk_rank_counts_{lbl.lower()}.csv"))
    ar.to_csv(output_path(f"bk_rank_avg_{lbl.lower()}.csv"))
...
for lbl, ad in [("CAT", ape_results["CAT"]), ("HDD", ape_results["HDD"]), ("CDD", ape_results["CDD"])]:
    ...
    print(f"\n  {lbl} APE\n{dff.to_string()}")
pd.concat(friedman_dfs, axis=1).to_csv(output_path("bk_friedman_nemenyi.csv"))
...
for lbl, ad in [("CAT", ape_results["CAT"]), ("HDD", ape_results["HDD"]), ("CDD", ape_results["CDD"])]:
    ...
    boot_dfs[lbl] = dfb
    print(f"done\n{dfb.to_string()}")
pd.concat(boot_dfs, axis=1).to_csv(output_path("bk_bootstrap_wilcoxon.csv"))

df_ape_cat.to_csv(output_path("bk_ape_cat.csv"))
df_ape_hdd.to_csv(output_path("bk_ape_hdd.csv"))
df_ape_cdd.to_csv(output_path("bk_ape_cdd.csv"))
df_mape.to_csv(output_path("bk_mape_cat.csv"))
df_rmse.to_csv(output_path("bk_rmse_hdd.csv"))
df_mae.to_csv(output_path("bk_mae_hdd.csv"))
df_wmape.to_csv(output_path("bk_wmape_cat.csv"))
checkpoint_forecast_exports(all_forecasts, obs_metadata)
